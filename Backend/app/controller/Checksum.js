...
class PaytmChecksum {
	...
	static getStringByParams(params) {
		...
		Object.keys(params).sort().forEach(function(key,value) {
			data[key] = (params[key] !== null && params[key].toString().toLowerCase() !== "null") ? params[key] : "";
		});
	...
	static calculateHash(params, salt) {		
		...
		return (crypto.createHash('sha256').update(finalString).digest('hex') + salt).substr(0,32);
	}
